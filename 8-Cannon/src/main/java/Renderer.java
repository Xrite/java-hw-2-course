...
class Renderer {
    ...
    Renderer(GraphicsContext context, double screenWidth, double screenHeight, double widthInMeters) {
        this.screenWidth = screenWidth;
        this.screenHeight = screenHeight;
        this.context = context;
        this.widthInMeters = widthInMeters;
        heightInMeters = screenHeight * widthInMeters / screenWidth;
    ...
    void drawCircle(Point2D center, double radius, Paint paint) {
    ...
    void drawLine(Point2D begin, Point2D end) {
    ...
    void fillUnderLine(Point2D begin, Point2D end, Paint paint) {
        context.setFill(paint);
        context.fillPolygon(new double[]{transformX(begin.getX()), transformX(end.getX()), transformX(end.getX()), transformX(begin.getX())},
                new double[]{transformY(begin.getY()), transformY(end.getY()), transformY(0), transformY(0)}, 4);
    }
    ...
    void fillBackground(Paint paint) {
        context.setFill(paint);
