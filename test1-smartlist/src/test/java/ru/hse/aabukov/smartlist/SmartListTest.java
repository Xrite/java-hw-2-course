...
public class SmartListTest {
    ...
    @Test
    public void checkOutOfBoundsException() throws Exception {
        List<Object> list = newList();
        for (int i = 0; i < 7; i++) {
            list.add(i + 1);
        }
        assertThrows(IndexOutOfBoundsException.class, () -> list.add(-1, 0));
        assertThrows(IndexOutOfBoundsException.class, () -> list.add(8, 0));
        ...
        assertThrows(IndexOutOfBoundsException.class, () -> list.set(7, 0));
        assertThrows(IndexOutOfBoundsException.class, () -> list.remove(7));
        assertThrows(IndexOutOfBoundsException.class, () -> list.remove(-1));
    }

    @Test
    public void checkAddByIndex() throws Exception {
        List<Object> list = newList();
        for (int i = 0; i < 3; i++) {
            list.add(i + 1);
        }
        ...
        list.add(1, 0);
        assertEquals(Arrays.asList(0, 0, 1, 2, 3), list);
        list.add(3, 0);
        assertEquals(Arrays.asList(0, 0, 1, 0, 2, 3), list);
        list.add(5, 0);
        assertEquals(Arrays.asList(0, 0, 1, 0, 2, 0, 3), list);
        list.add(7, 0);
        assertEquals(Arrays.asList(0, 0, 1, 0, 2, 0, 3, 0), list);
    }

    @Test
    public void checkAddToSingleByIndex() throws Exception {
        List<Object> list = newList();
        list.add(1);
        list.add(0, 0);
        ...
        list.add(1);
        list.add(1, 0);
        assertEquals(Arrays.asList(1, 0), list);
    }
    ...
    private static Class<?> getListClass() throws ClassNotFoundException {
        return Class.forName("ru.hse.aabukov.smartlist.SmartList");
    }
}
